...
#
# Modify by xujianfeng_sx@qiyi.com at 2016-06-01
#

"""
S3 Token And Transform Swift Account Use access key in Cache To AUTH_%(Cache[access]) Middleware

This WSGI component:

* Gets a request from the swift3 middleware with an S3 Authorization
  access key.
* If access key not in Cache Validates s3 token in Keystone and Set access key to Cache
* If access key in Cache, use Cached value to set tenant_name
...
from swift.common.utils import register_swift_info, config_true_value, get_logger


PROTOCOL_NAME = 'S3 Token Authentication And Transform Swift Account'
...
class S3Transformer(object):
    """Middleware that handles S3 authentication and transform account."""

    def __init__(self, app, conf):
        """Common initialization code."""
        self._app = app
        self._logger = get_logger(conf, log_route='s3_transformer')
        self._logger.debug('Starting the %s component', PROTOCOL_NAME)
        self._reseller_prefix = conf.get('reseller_prefix', 'AUTH_')

        self._use_account_cache = config_true_value(conf.get('account_cache', 'false'))
    ...
    def __call__(self, environ, start_response):
        ...
        self._logger.debug('Calling S3Transformer middleware.')
        ...
        if self._use_account_cache:
            new_tenant_name = _account_cache.get(access, None)
        if new_tenant_name is None:
            ...
            if self._use_account_cache:
                _account_cache[access] = new_tenant_name
...
def filter_factory(global_conf, **local_conf):
    ...
    register_swift_info("S3Transformer")

    def auth_filter(app):
        return S3Transformer(app, conf)
    return auth_filter
